...
/** Procedimento que desenha quadrados
    ...
    @post quadrado kom as dimendoes altura largura, desenhado a partir de
...
/** Representa um jogo de damas.
    @invariant posição destno e origem com valores de vector válidos,
...
private:
    ...
    /** Actualiza as informações de jogo (estatistica), consoante o
        ...
        @pre V.
        @post informações de peças actualizadas */
    inline void calculaNovasInformacoesDeJogo(Peca::Tipo const tipo_peca_tomada, bool const peca_tomada);
    /** Indica se a instância cumpre a condição invariante de classe.
        @pre V.
        @post posição destno e origem com valores de vector válidos,
    número de peças no tabuleiro válido */
    inline bool cumpreInvariante() const;  
    /** Envia a pilha_de_posições usando IPC++
        @pre V.
        @post pilha_de_posições enviada*/
    void despachaPilha();
    /** Executa os movimentos inerentes ás posições da pilha_de_poisições
        @pre pilha_de_posições devidamente recebida.
    ...
    Peca::Jogador meu_jogador;
    Mensageiro mensageiro;
    vector<Posicao> pilha_de_posicoes;
};
Peca::Peca(Tipo const tipo_da_peca,Jogador cor)
    :simulacao_a_decorrer(false),tipo_da_peca(tipo_da_peca), jogador_da_peca(cor)
{
    if (cor == brancas)
     ...
     ,peoes_pretas(12),damas_pretas(0),contador_de_jogadas(0)
{
    ...
    {
        meu_numero = rand();
        mensageiro.envia(meu_numero);
        mensageiro.leMensagem();
        numero_dele = mensageiro.mensagemLida<int>();
    }while(meu_numero == numero_dele);
    if (meu_numero > numero_dele)
    {
        meu_jogador = Peca::brancas;
        Aviso("Jogas com as Brancas").interage();
    }    
    else
    {
        meu_jogador = Peca::pretas;
...
{
    assert(cumpreInvariante());
    Posicao origem_(pilha_de_posicoes[0].linha(),pilha_de_posicoes[0].coluna());
    for(vector<Posicao>::size_type posicao_actual  = 1; posicao_actual != pilha_de_posicoes.size(); ++posicao_actual)
    { 
        Posicao destino_(pilha_de_posicoes[posicao_actual].linha(),pilha_de_posicoes[posicao_actual].coluna());
        Peca::Tipo peca_tomada = tabuleiro_de_jogo.executaJogada(origem_,destino_);
...
{
    assert(cumpreInvariante());
    int numero_de_posicoes = pilha_de_posicoes.size();
    mensageiro.envia(numero_de_posicoes);
    for(vector<Posicao>::size_type posicao_actual  = 0; posicao_actual != pilha_de_posicoes.size(); ++posicao_actual)
    { 
        int linha = pilha_de_posicoes[posicao_actual].linha();
        int coluna = pilha_de_posicoes[posicao_actual].coluna();
    ...
    ecra.refresca();
      do{ //ciclo referente ás 20 jogadas
        ...
        { // ciclo referente a troca de jogadores
            pilha_de_posicoes.clear(); //vector volta a Zero
           ...
           {
               mensageiro.leMensagem(); 
               int numero_de_posicoes = mensageiro.mensagemLida<int>();
              for(int posicao_actual = 0; posicao_actual != numero_de_posicoes; ++posicao_actual)
              { 
                  mensageiro.leMensagem(); 
                  int linha = mensageiro.mensagemLida<int>();
                  mensageiro.leMensagem(); 
                  int coluna = mensageiro.mensagemLida<int>();
                  pilha_de_posicoes.push_back(Posicao(linha,coluna));
              }
...
bool Jogo::seleccaoInteractiva()
{
    ...
    { 
        ...
        pilha_de_posicoes.push_back(origem); // adiciona a origem á pilha IPC
        destino = origem;
    }
    ...
    pilha_de_posicoes.push_back(destino); //adiciona + uma posicao a pilha IPC
    return true;
}
...
//c++ -Wall -ansi -pedantic -g -I/usr/local/include/stlport damas_entrega_final.C -lstlport_gcc_stldebug -lpthread -lefence -o damas_entrega_final -lslang -lSlang++ -lUtilitarios -lIPC++ 
